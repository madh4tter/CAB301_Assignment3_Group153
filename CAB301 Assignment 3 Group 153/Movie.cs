...
public class Movie : IMovie
{
    ...
    {
        Member instance = new Member(member.FirstName, member.LastName, member.ContactNumber, member.Pin);
        if (NoBorrowings >= 10)
        {
            Console.WriteLine("Maximum number of borrows hit");
            return false;
        }
        else if (AvailableCopies >= 1)
        {
            if (!Borrowers.Search(instance))
            {
                Borrowers.Add(instance);
                AvailableCopies--;
                NoBorrowings++;
                return true;
            }
            ...
            {
                Console.WriteLine("Member already in list");
                return false;
            }
        }
        else
    ...
    {
        //If the member is a borrower it deletes the member from the borrowers list and adds an available copy and takes the amount of borrowers
        bool searchResult = Borrowers.Search(member);
    ...
    public int CompareTo(IMovie movie)
    {
        //Gets the title from each movie and compares them to each other to see if they are in alphabetical order
        Movie another = (Movie)movie;

        if (title.CompareTo(another.title) < 0)
        {
            return -1;
        }
        else if (title.CompareTo(another.title) == 0)
        {
            return 0;
        }
        else
    ...
    {
        //Gathers each piece of data from the movie object and returns them
